...
docker compose --env-file .env.local --file=docker-compose.yml config 2>/dev/null | sed '/published:/ s/"//g' | sed "/^name:/d" | docker stack deploy --compose-file - ${COMPOSE_PROJECT_NAME}
...
#docker compose --env-file .env.local --file=docker-compose.override.yml config 2>/dev/null | sed '/published:/ s/"//g' | sed "/^name:/d" | docker stack deploy --compose-file - ${COMPOSE_PROJECT_NAME}
