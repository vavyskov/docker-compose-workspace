...
if [ ! -f .env.local ]; then
    ...
    if [ -n "$(cat < .env.local | grep TRAEFIK_HOSTNAME= | sed '/^#/d' | cut -d= -f2 | xargs)" ]; then
        TRAEFIK_HOSTNAME=$(cat < .env.local | grep TRAEFIK_HOSTNAME= | cut -d= -f2 | xargs)
    else
    ...
    if [ -n "$(cat < .env.local | grep ADMINER_HOSTNAME= | sed '/^#/d' | cut -d= -f2 | xargs)" ]; then
        ADMINER_HOSTNAME=$(cat < .env.local | grep ADMINER_HOSTNAME= | cut -d= -f2 | xargs)
    else
    ...
    if [ -n "$(cat < .env.local | grep MAILCATCHER_HOSTNAME= | sed '/^#/d' | cut -d= -f2 | xargs)" ]; then
        MAILCATCHER_HOSTNAME=$(cat < .env.local | grep MAILCATCHER_HOSTNAME= | cut -d= -f2 | xargs)
    else
